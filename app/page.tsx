...
async function fetchCandles(pair: string, timeframe: string, count: number = 100): Promise<Candle[]> {
  ...
  for (let i = count; i >= 0; i--) {
    ...
    const volatility = pair.includes("JPY")
      ? (tf.minutes >= 60 ? 1.5 : 0.3)
      : (tf.minutes >= 60 ? 0.01 : 0.0015);
...
function analyzeQuarterPoints(candles: Candle[]) {
  if (candles.length < 50) {
    return { quarterlyPoints: [], fibLevels: FIB_LEVELS, alignedPoints: [], strength: "WEAK" };
  }
  ...
  const quarterlyPoints: number[] = [rangeHigh, rangeLow];
  ...
  // Find aligned points (QP near Fib levels within 2% tolerance)
  const tolerance = range * 0.02;
  const alignedPoints: number[] = [];

  quarterlyPoints.forEach(qp => {
    fibLevels.forEach(fib => {
      if (Math.abs(qp - fib) < tolerance) {
        alignedPoints.push(qp);
      }
    });
  });
...
function calculateSignal(pair: string, candles: Candle[], timeframe: string): TradeSignal | null {
  ...
  // Check if event aligns with QP
  const qpTolerance = (resistance - support) * 0.02;
  const isNearQP = (price: number) =>
    qpAnalysis.alignedPoints.some(qp => Math.abs(price - qp) < qpTolerance);

  if (current.low < support && current.close > support * 0.999) {
    event = "SPRING";
    eventConfidence = Math.max(0.5, 1 - volumeRatio / 3);
    if (isNearQP(support)) eventConfidence = Math.min(1, eventConfidence + 0.2);
  } else if (current.high > resistance && current.close < resistance * 1.001) {
    event = "UPTHRUST";
    eventConfidence = Math.max(0.5, 1 - volumeRatio / 3);
    if (isNearQP(resistance)) eventConfidence = Math.min(1, eventConfidence + 0.2);
  } else if (current.low <= swingLow * 1.002 && current.close > current.open) {
    event = "TEST_SUPPORT";
    eventConfidence = 0.7;
    if (isNearQP(swingLow)) eventConfidence = Math.min(1, eventConfidence + 0.15);
  } else if (current.high >= swingHigh * 0.998 && current.close < current.open) {
    event = "TEST_RESISTANCE";
    eventConfidence = 0.7;
    if (isNearQP(swingHigh)) eventConfidence = Math.min(1, eventConfidence + 0.15);
  }
  ...
  const trs = recent.slice(-14).map(c =>
    Math.max(c.high - c.low, Math.abs(c.high - prev.close), Math.abs(c.low - prev.close))
  );
  const atr = trs.reduce((a, b) => a + b, 0) / trs.length;

  // QPT Bonus: +15% confidence when 2+ points align with Fib levels
  ...
  if (phase === "ACCUMULATION" && event === "SPRING" && volumeRatio < 1.5) {
    ...
    reasons = [
      timeframe + ": Accumulation (" + (phaseConfidence * 100).toFixed(0) + "%)",
      timeframe + ": Spring at support " + support.toFixed(5),
      "Volume: " + volumeRatio.toFixed(1) + "x average",
      "QPT: " + qpAnalysis.strength + " (" + qpAnalysis.alignedPoints.length + " aligned)",
      "R:R 1:" + riskReward + " (1% risk)",
    ...
    tp = current.close - stopDistance * riskReward;
    reasons = [
      timeframe + ": Distribution (" + (phaseConfidence * 100).toFixed(0) + "%)",
      timeframe + ": Upthrust at resistance " + resistance.toFixed(5),
      "Volume: " + volumeRatio.toFixed(1) + "x average",
      "QPT: " + qpAnalysis.strength + " (" + qpAnalysis.alignedPoints.length + " aligned)",
      "R:R 1:" + riskReward + " (1% risk)",
      "SL: " + sl.toFixed(5) + " | TP: " + tp.toFixed(5)
    ...
    if (signal?.qpAnalysis) {
      signal.qpAnalysis.fibLevels.forEach((fib: number, i: number) => {
        if (fib >= minPrice && fib <= maxPrice) {
          const alpha = i === 3 ? 0.3 : 0.15; // 50% level more visible
          ctx.strokeStyle = "rgba(92, 124, 250, " + alpha + ")";
          ctx.lineWidth = i === 3 ? 1.5 : 1;
          ctx.setLineDash([2, 4]);
          ctx.beginPath();
          ctx.moveTo(padding, priceToY(fib));
          ctx.lineTo(rect.width - padding, priceToY(fib));
          ctx.stroke();
        }
      });
      ctx.setLineDash([]);
    ...
    if (signal?.qpAnalysis?.alignedPoints) {
      signal.qpAnalysis.alignedPoints.forEach((qp: number) => {
        if (qp >= minPrice && qp <= maxPrice) {
          ...
          ctx.setLineDash([2, 2]);
          ctx.beginPath();
          ctx.moveTo(padding, priceToY(qp));
          ctx.lineTo(rect.width - padding, priceToY(qp));
          ctx.stroke();
        }
      });
      ctx.setLineDash([]);
...
export default function Dashboard() {
  ...
  const loadData = async (pair: string, tf: string) => {
    setLoading(true);
    const data = await fetchCandles(pair, tf, tf === "D1" ? 100 : 200);
    setChartData(data);
    setSignal(calculateSignal(pair, data, tf));
    setLoading(false);
  };
  ...
  const closePosition = (id: string) => {
    ...
    const trade: Trade = {
      ...
      exit,
      pnl,
      pnlPercent: pnl / (position.entry * 1000) * 100,
      reason: signal?.reasons[0] || "Trade closed",
    ...
    <div className="dashboard">
      ...
      <div style={{ display: "flex", gap: "16px", marginBottom: "20px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ position: "relative" }}>
          <input
            ...
            style={{ padding: "10px 14px", paddingLeft: "36px", background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: "8px", color: "var(--text-primary)", fontSize: "14px", width: "200px" }}
          />
          ...
          {showDropdown && (
            <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: "8px", maxHeight: "250px", overflowY: "auto", zIndex: 1000, marginTop: "4px" }}>
              {filteredPairs.map(pair => (
                <button
                  key={pair}
                  onClick={() => { setSelectedPair(pair); setSearchTerm(""); setShowDropdown(false); }}
                  style={{ display: "block", width: "100%", padding: "10px 14px", background: pair === selectedPair ? "var(--accent-blue)" : "transparent", border: "none", borderBottom: "1px solid var(--border-color)", color: "var(--text-primary)", textAlign: "left", cursor: "pointer", fontSize: "14px" }}
                >
                  {pair.replace("_", "/")}
                </button>
              ))}
        ...
        <div style={{ display: "flex", gap: "4px", background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: "8px", padding: "4px" }}>
          {TIMEFRAMES.map(tf => (
            <button
              ...
              style={{ padding: "6px 12px", background: timeframe === tf.value ? "var(--accent-blue)" : "transparent", border: "none", borderRadius: "6px", color: "var(--text-primary)", cursor: "pointer", fontSize: "13px", fontWeight: timeframe === tf.value ? "600" : "400" }}
            >
              {tf.label}
            </button>
          ))}
      ...
      <div className="card" style={{ marginBottom: "24px" }}>
        <div className="card-header">
          ...
          {signal?.qpAnalysis && (
            <span style={{ padding: "4px 12px", background: signal.qpAnalysis.strength === "STRONG" ? "var(--success-bg)" : signal.qpAnalysis.strength === "MODERATE" ? "var(--warning-bg)" : "var(--bg-secondary)", borderRadius: "4px", fontSize: "12px" }}>
              QPT: {signal.qpAnalysis.strength} ({signal.qpAnalysis.alignedPoints.length} Fib-aligned)
            </span>
          )}
        </div>
