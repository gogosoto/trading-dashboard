  ...
  return (
    <div className="dashboard">
      ...
      <div style={{ position: 'relative', marginBottom: '20px' }}>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '12px' }}>
          <div style={{ position: 'relative', flex: 1, maxWidth: '300px' }}>
            <input
              ...
              style={{ width: '100%', padding: '12px 16px', paddingLeft: '40px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', color: 'var(--text-primary)', fontSize: '14px' }}
            />
        ...
        {showDropdown && (
          <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', maxHeight: '300px', overflowY: 'auto', zIndex: 1000, marginTop: '4px' }}>
            {filteredPairs.map(pair => (
              <button key={pair} onClick={() => { setSelectedPair(pair); setSearchTerm(''); setShowDropdown(false); }} style={{ display: 'block', width: '100%', padding: '12px 16px', background: pair === selectedPair ? 'var(--accent-blue)' : 'transparent', border: 'none', borderBottom: '1px solid var(--border-color)', color: 'var(--text-primary)', textAlign: 'left', cursor: 'pointer', fontSize: '14px' }}>
                {pair.replace('_', '/')}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {FOREX_PAIRS.slice(0, 10).map(pair => (
            <button key={pair} onClick={() => setSelectedPair(pair)} className={`pair-tab ${selectedPair === pair ? 'active' : ''}`} style={{ padding: '6px 12px', fontSize: '12px' }}>
              {pair.replace('_', '/')}
            </button>
          ))}
        </div>
